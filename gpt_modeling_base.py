...
class GPT2BaseModel(BaseModel):
    ...
    def forward(self, input_ids, attention_mask, labels=None):
        ...
        return output.loss, output.logits  #output.logits: 模型的预测输出 
    ...
    def training_step(self, batch, batch_idx):
        ...
        if labels is not None and self.hparams.show_training_ex > -1 and batch_idx % self.hparams.show_training_ex == 0: #show_training_ex = -1 不打印训练例子
    ...
    def validation_step(self, batch, batch_idx): #batch_idx是当前批次的索引
        """ validation step """
        # 获取输入数据
        inputs = self.get_inputs(batch)
        batch_size = inputs["input_ids"].size(0)
        
        # 对模型进行前向传播计算的调用，获取损失和logits,其中inputs是输入数据的字典，logits是模型的输出。
        loss, logits = self(**inputs) 
    ...
    def validation_epoch_end(self, validation_step_outputs):  # validation_step_outputs:[{'loss': tensor(85.0330), 'num_total': 4, 'question': [...], 'answer': [...]}, {'loss': tensor(94.3365), 'num_total': 4, 'question': [...], 'answer': [...]}]
