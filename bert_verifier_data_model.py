...
class BertVerifierDataModel(BaseDataModel):
    ...
    @staticmethod
    def collate_fn(batch, args, tokenizer):  #base_dat_model.py 返回DataLoader
        batch_data = {}
        for key in batch[0]:  #batch : {'question': ,'ground_truth': ,'solution':, 'is_correct':}
            batch_data[key] = [example[key] for example in batch]
        # print("Keys in batch_data:", batch_data.keys())
        inputs_encoding = tokenizer(batch_data['question'], batch_data['solution'], add_special_tokens=True, return_tensors="pt", padding=True, truncation=True, max_length=512)  #inputs_encoding: {'input_ids': ,'token_type_ids': ,'attention_mask'}
        # token_type_ids这是BERT模型及其变种（例如RoBERTa、DistilBERT等）中引入的概念, 用于区分输入序列中的不同句子或片段
